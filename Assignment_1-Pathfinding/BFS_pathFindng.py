...
# Description:
# This program will use an open list and closed list approach to search
# for a path from the goal state "E" back to the start state "S".
# "X" are walls and cannot be traversed through while "0"
# are traversible nodes. If a node is traversed this way,
# from the starting point, each the nodes preceding the last
# one will be the parent node and so on and so forth until the goal is
# found. The program also outputs a results file that displays 
# all the "tried" nodes and marks the path found by the BFS

# Credits: 
# ChatGPT: for formatting, creating a psuedo code outline of
# the BFS algorithm and an explanation of how to use queues in python.

# Artificial Intelligence A Modern Approach 4th edition: for insight
# beyond my lecture notes for developing this method.
